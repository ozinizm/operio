      ...
      <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl relative z-10 animate-in zoom-in-95 duration-300 overflow-hidden border border-slate-100">
        <div className="p-8">
          ...
          <div className="space-y-4 mb-8">
            {/* E-posta */}
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 transition-all hover:border-primary/20">
              <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-primary">
                <Mail className="w-5 h-5" />
              </div>
              <div className="flex-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">E-posta</p>
                <p className="text-sm font-extrabold text-slate-800">{settings.support_email || 'info@fikircreative.com'}</p>
              </div>
              <a 
                href={`mailto:${settings.support_email || 'info@fikircreative.com'}`}
                className="p-2.5 bg-white shadow-sm hover:bg-primary/10 rounded-xl text-primary transition-all border border-slate-100"
              >
            ...
            {settings.support_whatsapp && (
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 transition-all hover:border-emerald-200">
                <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-emerald-500">
                  <MessageCircle className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">WhatsApp Hattı</p>
                  <p className="text-sm font-extrabold text-slate-800">+{settings.support_whatsapp}</p>
                </div>
                <a 
                  href={`https://wa.me/${settings.support_whatsapp.replace(/[^0-9]/g, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="p-2.5 bg-white shadow-sm hover:bg-emerald-100 rounded-xl text-emerald-600 transition-all border border-slate-100"
                >
                  <ExternalLink className="w-4 h-4" />
                </a>
              </div>
            ...
            {/* Çalışma Saatleri */}
            {settings.support_working_hours && (
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm font-extrabold text-slate-800">{settings.support_working_hours}</p>
                </div>
              </div>
            )}
          </div>
          ...
          {settings.support_emergency_note && (
            <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 flex gap-4 mb-8">
          ...
          <div className="flex gap-3">
            <Button 
              className="flex-1 h-14 rounded-2xl bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-tight shadow-lg shadow-primary/20 active:scale-95 transition-all"
              onClick={() => window.location.href = `mailto:${settings.support_email || 'info@fikircreative.com'}`}
              isLoading={isLoading}
            >
              Destek Al
            </Button>
            <Button 
              variant="outline"
              className="flex-1 h-14 rounded-2xl border-slate-200 text-slate-600 font-bold active:scale-95 transition-all"
              onClick={onClose}
            >
              Kapat
            </Button>
